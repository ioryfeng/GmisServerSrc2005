...
#End Region
    ...
    <WebMethod()> Public Function GetTransCondition(ByVal WorkflowID As String, ByVal projectID As String, ByVal TaskID As String) As DataSet
        ...
        Try
            ...
            Dim strSql As String = "{project_code=" & "'" & projectID & "'" & " and task_id=" & "'" & TaskID & "'" & " and isItem=1" & "}"
    ...
    '2003-04-11 by yanxuekui
    '定性评分--创建 项目定性分析记录
    <WebMethod()> Public Function CreateProjectCreditQuality(ByVal ProjectNo As String, ByVal CorporationNo As String, ByVal Phase As String) As Boolean
        Dim conn As New SqlConnection(strConn)
        conn.Open()
        Dim ts As SqlTransaction = conn.BeginTransaction
        Try
            Dim oQualityEvaluation As New BusinessRules.QualityEvaluation(conn, ts)
                CreateProjectCreditQuality = oQualityEvaluation.CreateProjectCreditQuality(ProjectNo, CorporationNo, Phase, "")
            ts.Commit()
    ...
    <WebMethod()> Public Function ImportFinanceData(ByVal CorporationCode As String, ByVal FromProjectCode As String, ByVal FromPhase As String, ByVal FromMonth As String, ByVal ToProjectCode As String, ByVal ToPhase As String, ByVal CreatePerson As String, ByVal CreateDate As DateTime, ByVal DeleteOriginalData As Boolean) As String
        Dim conn As New SqlConnection(strConn)
        conn.Open()
        Dim ts As SqlTransaction = conn.BeginTransaction
        Try
            Dim CommonQuery As New BusinessRules.CommonQuery(conn, ts)
                ImportFinanceData = CommonQuery.ImportFinanceData(CorporationCode, FromProjectCode, FromPhase, FromMonth, "", ToProjectCode, ToPhase, CreatePerson, CreateDate, DeleteOriginalData)
            ts.Commit()
            Return "1"
        Catch dbEx As DBConcurrencyException
            ts.Rollback()
